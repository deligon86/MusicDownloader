...
from src.views.Common.common_layouts import AutoColumnGrid
from src.views.Common.common_widgets import SongViewCardItem
...
class SongsView(BaseNormalScreenView):
    """
    View to display items based on the mode : available - songs, albums, foreign
    The display will be an AutoColumnGrid with SongViewCardItem widgets as children
    """

    mode = StringProperty()
    """
    :Attribute `mode` can be any of Songs|Albums|Foreign
    """
    ...
    def on_start(self):
        ...
        By default it loads in Songs Mode
        :return:
        """
        self.mode = "Songs"
    ...
    def change_view(self, view_name):
        """
        Change the sscreen content

        :param: view_name (str): The screen name to change to
        """
        ...
        async def start_widget_loading(view_, results_):
            if results_:
                ...
                for title_artist, data in results.items():
                    title, artist = title_artist.split("COL")
    ...
    def process_item(self, widget: SongViewCardItem, title, artist, link):
        """
    ...
    def _on_error(self, _, error):
        if error:
            self._notification_handler.post_notification(title="SongViewError",
                                                         message=error)
